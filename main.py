...
PLANS = {
            "normal": {"max_photos": 3, "model": os.environ.get("GEMINI_MODEL_NORMAL", "gemini-2.5-flash")},
            "pro": {"max_photos": 5, "model": os.environ.get("GEMINI_MODEL_PRO", "gemini-2.5-pro")},
}

@app.route("/health", methods=["GET"])
def health():
            return jsonify({"status": "ok", "models": {k: v["model"] for k, v in PLANS.items()}}), 200
...
def create_checkout():
            return jsonify({"error": "Stripe not configured yet"}), 400
